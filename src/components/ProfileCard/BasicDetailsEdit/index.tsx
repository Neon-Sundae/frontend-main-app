...
const ProfileImage: FC<IProfileImage> = ({ nfts, picture, setPicture, getNFTs }) => {
    ...
    >
      <div className={styles.wrapper}>
        ...
        <div className={styles.promptText}>
          ...
          <p>Set your profile picture to an NFT you own by connnecting your wallet and verifiying address</p>
          <div></div>
          <footer>By connecting your wallet you authorise Founder’s Lab to extract your NFT information belonging to <a>0xb23bvu3
            Learn More</a></footer>
          <button onClick={(e) => { e.preventDefault(); setStepTwo(true); setProfileModal(false); getNFTs() }}>Allow Access</button>
        </div>
      </div>
    </Modal >;
  }
  if (stepTwo) {
    ...
    return <Modal
      onClose={() => { setProfileModal(false); setStepTwo(false); }}
      width="650px"
      height="482.13px"
      overflowY={'auto'}
    >
  ...
  if (finStep) {
    return <Modal
      onClose={() => { setProfileModal(false); setStepTwo(false); setFinStep(false) }}
      width="599px"
      height="711px"
      overflowY={'auto'}
    >
          ...
          {nfts?.result.map((token: any) => {
            ...
            return (
              <><span className={styles.singleNft} key={getRandomString(5)} onClick={() => {
                console.log('clicked!');
                extractSelectedImageUri(metadata.image ? metadata.image.replace("ipfs://", "https://ipfs.io/ipfs/") : '');
              }}  >
                <img src={metadata.image ? metadata.image.replace("ipfs://", "https://ipfs.io/ipfs/") : userImage} alt="NFT" />
  ...
  return (
    <div className={styles['profile-image']} onClick={() => setProfileModal(true)}>
      <div className={styles['image-wrapper']}>
        <img alt="user" src={picture ? picture : userImage} className={styles.userImage} />
